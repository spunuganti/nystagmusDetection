...
delta = repmat(nanmax(nanmax(dm)), len, 1);
...
for i = 2:len                                               % start with the point of second highest rho and go in descending order
    ...
    [delta(rho.ord_sorted(i)), idx] = nanmin(dist);            % set delta to min dist, and idx to the index of the closest point with a higher rho 
