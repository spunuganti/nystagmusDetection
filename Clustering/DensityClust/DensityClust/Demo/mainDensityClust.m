...
dist = squareform(pdist(X,'correlation'));
...
[dc,~]  = choose_dc(dist);
[delta, clustInd, haloInd] = densityClust2(dist, dc, rho, isHalo);

% results = table();
% [results.numClust(:,1), results.clustInd(:,2), results.centInd(:,3), results.haloInd(:,4)] = densityClust(dist, dc, rho, isHalo);
